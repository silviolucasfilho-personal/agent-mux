...
- **Summary:** `trace_view::turn_summary(turn, observations)`: `── tokens ──` (input, output, cache read, cache write, total) and `── cost ──` (cost, with the count of unpriced generations when there are any, and generations), both taken from the turn's `trace_stats` row so they match the Turns pane; `── tools ──` (`N calls · M distinct`, then each tool with `×count`, most called first, ties by name). Tool rows and subagent launches (`agent: …`) are calls; a subagent's transcript row is not, so the total can be lower than the Turns pane's `🔧` count.
