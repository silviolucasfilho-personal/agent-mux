...
**Detail pane.** `query::list_observations(trace_id)` in chronological order. `v` cycles five views:
...
- **Summary:** `trace_view::turn_summary(turn, observations)`, laid out to fit the pane with no scrolling. `── tokens · N generations ──` (plus `· K unpriced` when some generations have no price), then input, output, cache read, cache write, total and cost, two to a row (one to a row when the pane is narrower than 44 columns), all taken from the turn's `trace_stats` row so they match the Turns pane. `── tools · N calls · M distinct · E errors ──` (the errors part only when there are any), then each tool with `×count` and `✗errors` (only when some of its calls failed), most called first, ties by name. A call counts as an error when its row's level is `ERROR`. Tool rows and subagent launches (`agent: …`) are calls; a subagent's transcript row is not, so the total can be lower than the Turns pane's `🔧` count, and the error total can differ from the Loop view's, which counts only tool rows. When the tools outnumber the rows left, `trace_view::fit_tools` keeps the most called ones and folds the rest into a last row, `… +N more · C calls · E errors`, so the totals still add up.
...
`src/persistence.rs` writes a pretty-printed JSON array of `{ profile, dir, skill_id?, conversation? }` (the full `Profile`, including tracing overrides) to `$AGENT_MUX_SESSIONS_FILE` or `~/.agent-mux/sessions.json` through a temp file and rename. `save_active_sessions` snapshots non-exited sessions after a dialog launch, kill, remove, resume, respawn, skill launch, restore, every PTY exit, and once more before `kill_all` at shutdown. Startup does not reconnect to old PTYs: each entry is respawned as a new child through `spawn_traced` with tracing replanned, the skill id preserved for singleton behaviour, and a missing directory falling back to the current one. `conversation` is the harness's own id for the conversation the session held: the launch's `session_key` in the trace store without its `<provider>:` prefix, or, until the store has correlated the launch, the id the session was resumed with. A restored entry that has one resumes it (`harness::restore_args`: the profile's own arguments with any earlier resume removed, then `--conversation <id>` for agy, `--resume <id>` for Claude, `resume <id>` for Codex), so the harness replays its dialog; an entry without one starts fresh as before. Missing or invalid files load as an empty list. Restored sessions become the initial selection.
...
| Trace Browser Detail (list, timeline, loop, summary) | `list_observations` | `SELECT * FROM observations WHERE trace_id = ?1 ORDER BY start_ns, rid` |
