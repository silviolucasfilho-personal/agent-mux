...
fn subagent_recovery_on_a_real_transcript() {
    ...
    let settings = MapSettings {
        provider: Provider::Claude, content_mode: ContentMode::Full,
        content_max_bytes: 20_000, redact_literals: vec![], user_id: None, release: None,
        tags: vec![], environment: None, profile_name: "Claude Code".into(),
        cwd: "/x".into(), project_slug: "-x".into(), agent_mux_session: 1,
        launch_id: "l".into(), run_id: "r".into(), correlation_plan: "d".into(),
        injected: false, attached: false, started_ns: 0,
    };
    ...
    for op in &ops {
        if let StoreOp::Observation(o) = op {
            total += 1;
            if o.parent_id.is_some() { parented += 1; }
            let is_sub = o.metadata.get("source").and_then(|v| v.as_str()) == Some("subagent");
            if is_sub {
                match o.obs_type {
                    ObservationType::Generation => {
                        sub_gen += 1;
                        if let Some(u) = &o.usage_raw {
                            tokens += u.iter().map(|(_, v)| *v).sum::<i64>();
                        }
                    }
    ...
    println!("observations emitted: {total}  with a parent: {parented}");
